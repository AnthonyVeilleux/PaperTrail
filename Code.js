...
function onOpen() {
  ...
  ui.showModelessDialog(
    HtmlService.createHtmlOutput('<p style="padding:8px;font-size:12px;"><strong>💡 Tip:</strong> Press <kbd style="background:#f0f0f0;padding:2px 6px;border-radius:3px;border:1px solid #ccc;">Shift+K</kbd> to toggle the Tag Manager sidebar.</p>')
      .setWidth(350)
      .setHeight(60),
...
/**
 * Extract all hashtags from the document
 */
...
function getAllTags() {
  try {
    ...
    projects.forEach(function(project) {
      ...
      if (project.tags) {
        project.tags.forEach(function(tag) {
          if (documentTags[tag.name]) {
            tag.count = documentTags[tag.name];
    ...
    globalTags.forEach(function(tag) {
      tag.count = documentTags[tag.name] || 0;
    });
...
/**
 * Test function to debug tag extraction
 */
